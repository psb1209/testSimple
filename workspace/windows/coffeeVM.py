...
def select_coffee(coffee):
    try:
        ...
        if money < price:
            messagebox.showwarning("잔액부족", f"{coffee} 가격은 {price}원 입니다.")
        else:
            charge = money - price
            label_result.config(
                text=f"{coffee}가 나왔습니다.\n잔돈: {charge}원"
            )
...
# 커피 버튼
btn_americano = tk.Button(
    root,
    text=f"아메리카노({menu['아메리카노']})원",
...
btn_caffuchino = tk.Button(
    root,
    text=f"카푸치노({menu['카푸치노']})원",
    command=lambda: select_coffee("카푸치노")
)
btn_caffuchino.grid(row=2, column=2, padx=10, pady=10)
